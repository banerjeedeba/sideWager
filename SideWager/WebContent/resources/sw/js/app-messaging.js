...
const saveToken = "http://sidewager-163802.appspot.com/rest/notifications/registerTopic"
var token;
...
messaging.onMessage(function(payload) {
  ...
  // TODO : appendMessage(payload);---------------------------------------------
  // [END_EXCLUDE]
});
// [END receive_message]


function resetMessagingUI() {
    //clearMessages();
    //showToken('loading...');
    // [START get_token]
...
function showToken(currentToken) {
  ...
  function registerTopic(topic)
  {
	  ...
	  console.log('token:'+token);
     	...
     	var url="https://www.sidewagerapp.com/rest/notifications/registerTopic?topic="+topic+"&ckey="+token;
  ...
  function sendMsg(topic){
	  ...
	  console.log('msg topic:'+topic);
     	var url="https://www.sidewagerapp.com/rest/notifications/sendTopic?topic="+topic;
 	   	...
 	   	$.ajax({url: url,
 	   		...
 	   		success: function(data){
 	   			...
 	   			$.each(data,function(key,value){
 	   				if(value.error != undefined){
 	   				alert("msg error..."+value.error);
	   	   	    		return;
 	   				}else{
 	   					alert("msg success..."+value.success);
 	   				}
 	   	    		
 	   			});
	   	    },
 	    	error: function(xhr,status,error){
 	    		alert("msg error..."+error+"--"+status);
